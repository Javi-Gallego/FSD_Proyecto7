...
export const TablonTimeline = ({ tablon }) => {
    ...
    return (
        <div className="tablonDesign">
            <div key="tablon">
                {usedTablon && usedTablon.map((post, index) => (
                    <div key={`message${index}`} className="mensajeDesign">
                        ...
                        <div key={`likescomments${index}`} className="minorText">
                            ...
                            {post.likes && post.likes.map((like, indexlike) => (
                                likeString.concat(like.userName + " ")                            
                            ))}
                            <br /><br />
                            <img src={commentIcon} onClick={ () => 
                                {handleCommentClick(post._id, index) 
                            }}/>Comentarios({post.comments.length}):
                            {post.comments && post.comments.map((comment, indexcomment) => (
                                <div className="comentary" key={`commentary${indexcomment}`}>{comment.commentatorId.userName} :  {comment.commentary} </div>                      
                            ))}
                            <div id={`blockComment${index}`} className="addComment invisible">
                                <MyInput
                                    type="text"
                                    name="comment"
